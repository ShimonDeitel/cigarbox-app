...
final class Store: ObservableObject {
    @Published var entries: [CigarEntry] = []
    ...
    private var supportDirectory: URL {
        ...
        let dir = fm.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0]
            .appendingPathComponent("Cigarbox", isDirectory: true)
    ...
    private func seed() {
        entries = [
            CigarEntry(name: "Padron 1964", brand: "Padron", size: "Toro", rating: "4.8"),
            CigarEntry(name: "Hemingway", brand: "Arturo Fuente", size: "Short Story", rating: "4.3")
        ]
    }
    ...
    func load() {
        ...
        if let data = try? Data(contentsOf: entriesURL),
           let decoded = try? JSONDecoder().decode([CigarEntry].self, from: data) {
            entries = decoded
        }
    ...
    func add(name: String, f1: String, f2: String, f3: String) -> Bool {
        ...
        let entry = CigarEntry(name: name, brand: f1, size: f2, rating: f3)
        entries.insert(entry, at: 0)
        save()
        return true
    }

    func update(_ entry: CigarEntry) {
    ...
    func delete(_ entry: CigarEntry) {
